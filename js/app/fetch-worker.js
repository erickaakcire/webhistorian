...
self.fetchVisits = function()
{
	if (self.itemsList.length > 0) {
		...
		self.postMessage({ "action": "fetchHistory", "historyItem": historyItem});
	}
};
...
//		self.postMessage({ "action": "updateProgress", "count": self.itemsList.length, "earliest": self.earliestTimestamp, "latest": self.latestTimestamp });
		...
		else if (self.finished == false)
		{
			...
			self.finished = true;
			self.postMessage({ "action": "updateProgress", "items": self.fullData, "count": self.itemsList.length, "earliest": self.earliestTimestamp, "latest": self.latestTimestamp, "finished": true });
		}
	};
